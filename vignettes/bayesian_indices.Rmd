...
compute_model <- function(data, outcome_type="linear", type=c("frequentist", "bayesian"), chains=4, iter=2000, warmup = 1/2){
  ...
  if("bayesian" %in% c(type)){
    ...
    params$p_direction_AUC <- p_direction(posterior, raw=FALSE)
    ...
    # Bayesfactor
    params$bayesfactor_0707 <- as.numeric(bayesfactor_savagedickey(posterior, 
                                      prior = rcauchy_perfect(length(posterior), location=0, scale=0.707), 
                                      hypothesis = 0))
    
    params$bayesfactor_1 <- as.numeric(bayesfactor_savagedickey(posterior, 
                                      prior = rcauchy_perfect(length(posterior), location=0, scale=1), 
                                      hypothesis = 0))
...
chains <- seq(1, 2)
iterations <- seq(100, 500, by=100)
warmups <- seq(4/10, 6/10, by=1/10)
# Run --------------------------------------------------------------------------
# Initialize data
all_data <- data.frame()
